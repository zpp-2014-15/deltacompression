...
Przewa¿nie s¹ one wykonywane w obrêbie jednego centrum danych.
Mo¿na jednak zapewniæ znacznie wiêksz¹ odpornoœæ na b³êdy, wystarczy wykonaæ replikê w innym centrum danych.
Niestety du¿ym problemem tego rozwi¹zania jest wydajnoœæ ³¹cza internetowego.
Zwyk³e przes³anie danych mog³oby zaj¹æ zbyt du¿o czasu.
Istniej¹ jednak algorytmy, które pozwalaj¹ na znaczne zmniejszenie rozmiaru danych potrzebnych do przes³ania.
Jednym z takich algorytmów jest delta kodowanie.
Firma 9LivesData jest twórc¹ jednego z najszybszych i najlepszych systemów kopii zapasowych.
Zaimplementowanie nowego rozwi¹zania w istniej¹cym systemie wymaga du¿ego nak³adu pracy.
Z tego powodu zosta³ stworzony symulator, który naœladuje pracê systemu firmy 9LivesData oraz zawiera delta kodowanie.
Dziêki niemu jest mo¿liwe testowanie nowych pomys³ów przed ich pracoch³onn¹ implementacj¹ w produkcyjnym systemie.
...
\section{Kompresja}
Przed przes³aniem bloki s¹ poddawane dzia³aniu kompresji, co pozwala znacz¹co zmieniæ rozmiar danych.
...
\begin{figure}[h]
    ...
    \caption{System A dzieli dane na bloki, dla ka¿dego z bloków liczy wartoœæ funkcji skrótu. Dla identycznych bloków dostajemy takie same wartoœci.}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{System A dzieli dane na bloki i liczy wartoœci funkcji skrótu. Zmieni³y siê tylko dwa bloki, co jest zas³ug¹ Chunkera.}
\end{figure}
