...
Jest jednym z najwiêkszych i najszybszych takich systemów na œwiecie u¿ywanym przez klientów w Japonii i Stanach Zjednoczonych (zgodnie z \cite{HYDRAstor}).
Zosta³ stworzony, aby dostarczyæ klientom funkcjonalnoœci takie, jak: deduplikacja, bloki zmiennej d³ugoœci i pe³ny indeks bloków u¿ywaj¹cy rozproszonej tablicy mieszaj¹cej.
Ponadto system charakteryzuje siê du¿¹ odpornoœci¹ na b³êdy.

System HYDRAstor jest niezwykle z³o¿ony i skomplikowany, dlatego zaprezentujemy go w du¿ym uproszczeniu.
...
Chunker zamienia strumieñ na bloki zmiennej d³ugoœci.
Bloki wraz z metadanymi s¹ dostarczane do systemu HYDRAstor poprzez po³¹czenie lokalne.
System HYDRAstor znajduje wartoœci funkcji skrótu dla ka¿dego z bloków i przechowuje jedynie unikalne bloki.
Proces ten nosi nazwê deduplikacji.
Aby zaktualizowaæ wersjê systemu na zdalnym serwerze, niezbêdne dane s¹ kompresowane i przesy³ane przez sieæ globaln¹ (replikacja).
\begin{figure}[h]
    \centering
...
Na serii rysunków \ref{fig:hydra1} - \ref{fig:hydra8} mo¿emy zaobserwowaæ, jakie akcje wykonuje system przy tworzeniu dwóch kolejnych kopii zapasowych.
Przed pierwsz¹ kopi¹ w systemie nie by³o ¿adnych danych.
Natomiast w drugiej kopii zostaj¹ dodane dwa bajty w œrodku strumienia danych.

Skupimy siê teraz na replikacji.
Bêdziemy rozwa¿aæ wp³yw bloków zmiennej d³ugoœci, deduplikacji i kompresji na rozmiar przesy³anych danych.
Na koniec powiemy, w jaki sposób delta kodowanie mo¿e niezale¿nie od istniej¹cych mechanizmów zwiêkszyæ efektywnoœæ replikacji.
\section{Bloki zmiennej d³ugoœci}
Bloki zmiennej d³ugoœci pozwalaj¹ w znacznym stopniu zmniejszyæ rozmiar danych, które zostan¹ przes³ane podczas replikacji.
...
Sprawia to, ¿e po³owa strumienia zostanie podzielona na zupe³nie inne bloki, mimo ¿e dane siê praktycznie nie zmieni³y.

Stosowanie bloków zmiennej d³ugoœci w po³¹czeniu z komponentem zwanym Chunkerem, który wybiera granice bloków na podstawie samych danych, pozwala unikn¹æ tego problemu. 
...
\section{Kompresja}
Przed przes³aniem bloki s¹ poddawane dzia³aniu kompresji, co pozwala znacz¹co zmniejszyæ rozmiar danych.
Stosowana kompresja musi nie tylko mieæ dobry stopieñ, ale te¿ nie zu¿ywaæ zbyt du¿o zasobów systemu.
Du¿e znaczenie ma te¿ to, jak wiele danych kompresujemy na raz.
Korzystniej jest kompresowaæ du¿e fragmenty danych.
Nale¿y pamiêtaæ jednak, ¿e w wiêkszym stopniu obci¹¿a to system.
\section{Potencjalne korzyœci z delta kodowania}
...
W sytuacjach, gdy ró¿nice pomiêdzy blokami s¹ ma³e, delta kodowania potrafi znacznie zmniejszyæ nadmiarowoœæ danych.
Przechowywanie zbioru ró¿nic jest zdecydowanie bardziej efektywne pamiêciowo ni¿ przechowywanie ca³ych bloków.
Oczywiœcie, na ich wielkoœæ ma bardzo du¿y wp³yw to, które dane ze sob¹ porównujemy, a tak¿e jak liczymy ró¿nice.
...
Ograniczymy siê jednak do xdelta3, którego u¿ywamy w symulatorze.
...
\State $best\_block \gets blocks[1]$
\State $best\_diff \gets \Call{Diff}{block, blocks[1]}$
