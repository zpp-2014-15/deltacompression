...
W celu przes³ania kopii zapasowej na zdalny serwer nowe dane s¹ kompresowane i przesy³ane przez sieæ Internet (replikacja).
...
Pomaga to równie¿ w replikacji, poniewa¿ liczba bloków oraz rozmiar danych wymaganych do przes³ania jest znacznie mniejszy.
Ma to przede wszystkim du¿e znaczenie, gdy kopie s¹ wykonywane czêsto i dane w wiêkszoœci siê nie zmieniaj¹.
...
Dla ka¿dego bloku bêdziemy liczyæ okreœlon¹, sta³¹ iloœæ $S \times K$ wartoœci liczbowych zwanych cechami (ang. \textit{feature}).
...
\begin{description}
        \item[Wybór sposobu przechowywania danych.] \hfill
        
        Mo¿na wybraæ miêdzy u¿ywaniem wy³¹cznie pamiêci RAM a wykorzystaniem bazy danych.
	...
	Symulator umo¿liwia liczenie, ile szacunkowo odczytów i zapisów na dysk wykonuje dla danej wersji ka¿dy z algorytmów delta kodowania.
...
W tym rozdziale zostan¹ przedstawione i omówione rezultaty eksperymentów.
...
Dziêki symulacjom na ró¿nych danych mo¿na przewidzieæ jakie wyniki bêd¹ dawaæ testowane algorytmy w œrodowisku produkcyjnym.
...
Zosta³y przygotowane trzy wersje.
Kolejne wersje Ÿróde³ j¹dra Linuxa by³y oddalone od siebie o piêæ tysiêcy zmian (ang. \textit{commit}).
...
\item Zielony - brak delta kodowania, kompresja LZO. Warto dodaæ, ¿e jest to obecny wynik systemu HYDRAstor
...
Ka¿da grupa sk³ada siê z szeœciu s³upków.
Pierwsza grupa pokazuje, ile danych trzeba przes³aæ dla pierwszej wersji danych przy u¿yciu ka¿dej z metod, druga grupa dla drugiej wersji, a trzecia grupa dla trzeciej wersji Ÿróde³ j¹dra Linuxa.

Zacznijmy od iloœci duplikatów, które zosta³y wykryte podczas symulacji.
Dla pierwszej wersji jest to tylko 0.05\%.
Prawie wszystkie dane, które otrzyma³ system by³y zupe³nie nowe.
...
Wszystkie trzy nie u¿ywaj¹ indeksu podobieñstwa, ró¿ni¹ siê jedynie typem kompresji u¿ytej przed wys³aniem danych.
...
Dla pierwszej wersji wyniki braku delta kodowania z ró¿nymi typami kompresji (kolory czerwony, zielony, ¿ó³ty) s¹ prawie równe wynikom metody z u¿yciem indeksu podobieñstwa (kolory b³êkitny, fioletowy, szary).
Brak poprawy w stosunku do algorytmu bez delta kodowania wynika ze zbyt ma³ej iloœci bloków w systemie, aby by³o mo¿liwe znalezienie podobnych bloków.
Jednak do przes³ania drugiej i trzeciej wersji Ÿróde³ j¹dra Linuxa rozmiar wynikowych danych jest znacznie mniejszy.
Odpowiednio dla braku delta kodowania i kompresji LZO jest to 57MB, a dla indeksu podobieñstwa 26MB.
...
Niestety delta kodowanie radzi sobie znacznie gorzej ni¿ w poprzednim eksperymencie.
...
Najczêœciej dodaje siê b¹dŸ usuwa ca³e pliki, rzadziej modyfikuje siê istniej¹ce pliki.
...
Kolejnymi danymi, na których testowaliœmy algorytmy delta kodowania by³y Ÿród³a projektu Django.
ród³a Django zajmuj¹ tylko kilkadziesi¹t megabajtów i dlatego by³o mo¿liwe przetestowanie optymalnego algorytmu delta kodowania.
To pozwoli³o nam odpowiedzieæ na pytanie, jak dobrze dzia³a indeks podobieñstwa wzglêdem optymalnego algorytmu delta kodowania.
...
\begin{figure}[H]
    ...
    \label{fig:torch}
 \end{figure}
Równie¿ jak poprzednio przygotowaliœmy trzy wersje danych.
Liczba duplikatów by³a mniejsza ni¿ 1\% dla ka¿dej wersji.
Dla drugiej wersji indeks podobieñstwa zmniejszy³ dane tylko o 2\%.
Jest to bardzo s³aby wynik w porównaniu do algorytmu optymalnego, który zmniejszy³ rozmiar danych o prawie 30\%.
Dla trzeciej wersji danych testowych indeks podobieñstwa zmniejszy³ iloœæ danych o 9\%, a algorytm optymalny o 37\%.
Wynika z tego, ¿e lepsze dobranie parametrów indeksu podobieñstwa mog³oby znacznie poprawiæ jego wyniki.
W szczególnoœci nale¿y zwróciæ uwagê na d³ugoœæ okna oraz liczbê cech.
Zmniejszenie d³ugoœci okna oraz zwiêkszenie liczby cech mog³oby znacznie poprawiæ wynik indeksu podobieñstwa w stosunku do wyniku algorytmu optymalnego.
Oczywiœcie wynik indeksu podobieñstwa nie mo¿e byæ lepszy od wyniku algorytmu optymalnego.
Wynik algorytmu optymalnego jest dolnym oszacowaniem na wynik indeksu podobieñstwa.
...
\multicolumn{1}{ |c| }{Brak} & 658MB & 1052MB & 660MB & 79.48MB    \\ 
\cline{2-6}
\multicolumn{1}{ |c  }{}                        &
\multicolumn{1}{ |c| }{LZO} & 174MB & 271MB & 450MB & 19.24MB  \\ 
\cline{2-6}
\multicolumn{1}{ |c  }{}                        &
\multicolumn{1}{ |c| }{LZMA} & 103MB & 160MB & 400MB & 12.35MB \\ 
\cline{1-6}
\multicolumn{1}{ |c  }{\multirow{3}{*}{Indeks podobieñstwa} } &
\multicolumn{1}{ |c| }{Brak} & 396MB & 941MB & 566MB & 76.33MB     \\ 
\cline{2-6}
\multicolumn{1}{ |c  }{}                        &
\multicolumn{1}{ |c| }{LZO} & 110MB & 248MB & 446MB & 18.72MB      \\ 
\cline{2-6}
\multicolumn{1}{ |c  }{}                        &
\multicolumn{1}{ |c| }{LZMA} & 68MB & 149MB & 400MB & 12.11MB      \\ 
\cline{1-6}
\multicolumn{1}{ |c  }{\multirow{3}{*}{Optymalne} } &
\multicolumn{1}{ |c| }{Brak} & N/A & N/A & N/A & N/A     \\ 
\cline{2-6}
\multicolumn{1}{ |c  }{}                        &
\multicolumn{1}{ |c| }{LZO} & N/A & N/A & N/A & 14.6MB      \\ 
\cline{2-6}
\multicolumn{1}{ |c  }{}                        &
...
Tabelê powinno siê analizowaæ kolumnami. Jedna kolumna przedstawia wyniki dla jednych danych testowych przy u¿yciu ró¿nych metod replikacji.
...
Badano tam m. in. sposób odwzorowywania super cech na bloki - porównywano indeks pe³ny, czêœciowy oraz przechowywanie informacji w pamiêci podrêcznej LRU, co opiera³o siê na za³o¿eniu, ¿e podobne bloki bêd¹ siê znajdowaæ obok siebie.
...
Wiêksza liczba super cech na blok zwiêksza szanse na dopasowanie bloku.
Taki sam wp³yw ma ma³y rozmiar bloku - dla tego samego rozmiaru danych testowych liczba bloków jest wiêksza.
Dziêki temu równie¿ jest ³atwiej znaleŸæ podobny blok.
Kolejn¹ ró¿nic¹ jest rozmiar danych.
Nasze dane testowe by³y rzêdu gigabajtów, w pracy Philipa Shilane'a rzêdu terabajtów.
Inaczej te¿ wykonywaliœmy kompresjê, u nas mia³a ona charakter globalny, a nie lokalny.
Z tego powodu zysk z kompresji by³ wiêkszy, a z delta kodowania mniejszy.
Dysponowaliœmy tak¿e zupe³nie innym Chunkerem.
