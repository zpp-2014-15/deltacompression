...
Przesy³anie zbioru ró¿nic jest zdecydowanie bardziej efektywne pamiêciowo ni¿ przesy³anie ca³ych bloków.
Oczywiœcie, na ich wielkoœæ ma bardzo du¿y wp³yw to, które dane ze sob¹ porównujemy, a tak¿e to, jak liczymy ró¿nice.
...
Problem ten rozwi¹zuje popularny algorytm Diff z systemów Unix.
Jednak nie sprawdzi siê on w przypadku delta kodowania.
Algorytm Diff porównuje bloki na poziomie linii, co ma szanse sprawdziæ siê jedynie dla specyficznego rodzaju danych tekstowych.
...
Mamy zbiór starych bloków i jeden nowy blok.
Chcemy znaleŸæ stary blok, dla którego ró¿nica z nowym blokiem jest mo¿liwie ma³a.
Jednoczeœnie algorytm powinien mieæ dobr¹ z³o¿onoœæ czasow¹.
...
Jest on niemo¿liwy do zastosowania w praktyce przez asymptotycznie liniowy czas dzia³ania ze wzglêdu na liczbê bloków w systemie (która jest rzêdu milionów).
Jednak jest on dobrym punktem odniesienia do innych algorytmów - daje on informacjê, jakie jest dolne ograniczenie na rozmiar przesy³anych danych w klasie algorytmów delta kodowania porównuj¹cych z jednym blokiem.
\begin{algorithmic}
...
Wartoœciami $a_i$ oraz $m_i$ powinny byæ du¿e, losowe liczby pierwsze, jednak u nas bêd¹ ustalone, aby zapewniæ powtarzalnoœæ wyników.
Nastêpnie tworzymy super cechy (ang. \textit{superfeature}) z ustalonej liczby $S$ zwyk³ych cech.
W tym celu dzielimy cechy danego bloku na grupy po $K$ kolejnych cech i dla ka¿dej z grup liczymy znacznik Rabina - jego wartoœæ bêdzie wartoœci¹ danej super cechy.
Wszystkie znaczniki liczymy przy u¿yciu du¿ych liczb pierwszych $P$ i $Q$.
Aby znaleŸæ podobny blok do danego, wybieramy taki, który ma najwiêcej wspólnych super cech (w przypadku remisu wybieramy dowolny).
