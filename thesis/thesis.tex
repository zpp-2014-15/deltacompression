...
Zosta³ stworzony, aby dostarczyæ klientom funkcjonalnoœci takie, jak: deduplikacja, samozarz¹dzanie, mo¿liwoœæ konfiguracji wed³ug potrzeb. Ci¹g³e aktualizacje nie przerywaj¹ dzia³ania produktu. Ponadto system charakteryzuje siê du¿¹ odpornoœci¹ na b³êdy (patrz \cite{HYDRAstor}).
\\*
...
\State $features, best \gets \Call{initializeFeatures}{S \times K}$
\ForAll{window $w$ in $block$}
    \ForAll{$feature_{i}, best_{i}, m_{i}, a_{i}$}
        \State $value \gets m_{i} \times \Call{fp}{w} + a_{i} \mod 2^{32}$
        \If {$value < best_{i}$}
            \State $best_{i} \gets value$
            \State $feature_{i} \gets \Call{fp}{w}$
        \EndIf
    \EndFor
\EndFor
...
\For{$hash \ \textbf{in} \ hashes$}
    ...
    \EndIf
\EndFor
\State $sfeatures\_map.add(block.getHash(), sfeatures)$
\For{$sfeature \ \textbf{in} \ sfeatures$}
...
Symulator zosta³ napisany g³ównie w jêzyku Python w wersji 2.7. W jêzyku C++ zosta³ napisany adapter Chunkera. Z kolei wymagaj¹ca obliczeniowo czêœæ delta kodowania z indeksem podobieñstwa zosta³a napisana w C.
...
Na p³ycie CD-ROM do³¹czonej do niniejszej pracy znajduj¹ siê:
	\begin{itemize}
		\item
			Kod Ÿród³owy symulatora
		\item 
			Skrypt przygotowuj¹cy dane testowe
		\item
			Dokumentacja wraz z opisem instalacji
		\item 
			Kod Ÿród³owy pracy licencjackiej
	\end{itemize}
\end{document}
